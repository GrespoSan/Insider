...
st.title("Independent Insider Radar — v0.1")
...
st.info(
    "Regola v0.1: Form 4 originale, transazione non-derivata con codice P e A (acquired), "
...
with c1:
    if st.button("1. Scarica / aggiorna SEC", type="primary", use_container_width=True):
        if not email or "@" not in email:
            ...
            if missing:
                st.warning("Non ancora pubblicati: " + ", ".join(missing))
            if errors:
                st.error("\n".join(errors))
...
if isinstance(summary, pd.DataFrame) and not summary.empty:
    ...
    st.warning(
        "v0.1 riporta statistiche descrittive, non significatività robusta. "
